...
5. 新增校准指标：ECE、Brier Score、Reliability Diagram
...
def plot_reliability_diagram(y_true, y_prob, save_path, n_bins=15, title="Reliability Diagram"):
    """
    绘制Reliability Diagram并保存，同时展示各置信度区间的样本数分布。
    """
    confidences = np.max(y_prob, axis=1)
    predictions = np.argmax(y_prob, axis=1)
    accuracies = (predictions == y_true).astype(float)
    ...
    non_empty = np.array(bin_counts) > 0

    fig, (ax_top, ax_bottom) = plt.subplots(
        2, 1, figsize=(8, 9), sharex=True, gridspec_kw={'height_ratios': [3, 1]}
    )
    ax_top.plot([0, 1], [0, 1], 'k--', label='Perfectly calibrated')
    ax_top.bar(
        np.array(bin_centers)[non_empty],
        np.array(bin_accuracies)[non_empty],
        width=1.0 / n_bins,
        alpha=0.55,
        edgecolor='black',
        color='steelblue',
    ...
    ax_top.plot(
        np.array(bin_confidences)[non_empty],
        np.array(bin_accuracies)[non_empty],
        marker='o',
        color='tab:orange',
        linewidth=1.5,
    ...
    ax_top.set_ylabel('Accuracy', fontsize=14)
    ...
    ax_bottom.set_xlabel('Confidence', fontsize=14)
...
def plot_confusion_matrix(all_labels, all_preds, save_path, title="Confusion Matrix", class_names=None):
    """绘制并保存混淆矩阵"""
...
def main():
    ...
    for model_name, protocol in experiment_jobs:
        ...
        try:
            ...
            # 存储五折结果
            fold_results = []
            all_fold_image_preds = []
            all_fold_image_labels = []
            all_fold_image_probs = []
            all_fold_patient_preds = []
            all_fold_patient_labels = []
            ...
            for fold_idx, (train_pids, val_pids) in enumerate(patient_stratified_kfold(patient_data, n_splits=5)):
                ...
                plot_confusion_matrix(all_labels, all_preds,
                                      os.path.join(fold_save_path, f"confusion_matrix_image_fold{fold_idx+1}.png"),
                                      title=f"Image-Level CM - Fold {fold_idx+1}", class_names=NEW_CLASS_NAMES)
                # 患者级混淆矩阵
                plot_confusion_matrix(patient_true, patient_pred,
                                      os.path.join(fold_save_path, f"confusion_matrix_patient_fold{fold_idx+1}.png"),
                                      title=f"Patient-Level CM - Fold {fold_idx+1}", class_names=NEW_CLASS_NAMES)
                # Reliability Diagram
                plot_reliability_diagram(all_labels, calibrated_probs,
                                         os.path.join(fold_save_path, f"reliability_diagram_fold{fold_idx+1}.png"),
                                         title=f"Calibrated Reliability Diagram - Fold {fold_idx+1}")
                ...
                # 汇总所有折的预测
                all_fold_image_labels.extend(all_labels.tolist())
                all_fold_image_preds.extend(all_preds.tolist())
                all_fold_image_probs.extend(calibrated_probs.tolist())
                all_fold_patient_labels.extend(patient_true)
                all_fold_patient_preds.extend(patient_pred)
            ...
            # 绘制总体混淆矩阵
            if len(all_fold_image_labels) > 0:
                plot_confusion_matrix(np.array(all_fold_image_labels), np.array(all_fold_image_preds),
                                      os.path.join(model_save_dir, "overall_confusion_matrix_image.png"),
                                      title=f"Overall Image-Level CM - {model_name}", class_names=NEW_CLASS_NAMES)
            if len(all_fold_patient_labels) > 0:
                plot_confusion_matrix(np.array(all_fold_patient_labels), np.array(all_fold_patient_preds),
                                      os.path.join(model_save_dir, "overall_confusion_matrix_patient.png"),
                                      title=f"Overall Patient-Level CM - {model_name}", class_names=NEW_CLASS_NAMES)
